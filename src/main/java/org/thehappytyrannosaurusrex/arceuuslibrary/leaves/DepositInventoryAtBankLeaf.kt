...
class DepositInventoryAtBankLeaf(script: ArceuusLibrary) :
    ...
    private fun updateProgressAndMaybeFail(distNow: Double) {
        ...
        if (shrink >= REQUIRED_PROGRESS_TILES) {
            ...
            if (noProgressTries > 0) {
                Logger.info("[DepositInv:Progress] Distance decreased by ${"%.1f".format(shrink)} tiles (now=${"%.1f".format(distNow)}). Resetting attempts.")
            }
            noProgressTries = 0
            return
        }
        if (windowElapsed) {
            noProgressTries++
            Logger.info("[Failsafe] Bank no-progress attempt $noProgressTries/$MAX_NO_PROGRESS_TRIES (dist=${"%.1f".format(prev)}→${"%.1f".format(distNow)} in ~${NO_PROGRESS_WINDOW_MS/1000}s; need ≥${"%.0f".format(REQUIRED_PROGRESS_TILES)}).")
            lastDistance = distNow
            lastProgressAt = now
    ...
    override fun execute() {
        if (!hasNonBookItems()) {
            Logger.info("[DepositInv] No non-book items left; skipping bank.")
            noProgressTries = 0
            return
        }
        ...
        if (Bank.present()) {
            noProgressTries = 0
            if (!Bank.opened()) {
                Logger.info("[DepositInv] Bank nearby, attempting to open.")
                Bank.open()
                return
            }
            if (Inventory.isNotEmpty()) {
                Logger.info("[DepositInv] Depositing full inventory.")
                Bank.depositInventory()
            } else {
                Logger.info("[DepositInv] Inventory already empty; nothing to deposit.")
            }
            return
        }
