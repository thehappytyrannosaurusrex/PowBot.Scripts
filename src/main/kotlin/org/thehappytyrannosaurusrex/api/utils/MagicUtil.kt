...
object MagicUtil {
    ...
    // Cost of a spell in terms of a single rune type
    ...
    // True if spell is ready to cast (optionally ensures tab is open)
    fun isReadyToCast(
    ...
    fun hasAncientAccess(): Boolean = Quests.Quest.DESERT_TREASURE_I.completed()
    fun hasLunarAccess(): Boolean = Quests.Quest.LUNAR_DIPLOMACY.completed()
    fun canUseAncient(): Boolean = hasAncientAccess()
    fun canUseLunar(): Boolean = hasLunarAccess()

    // --- Core rune / cast math ---

    // Compute how many casts possible given rune counts and costs
    ...
    // Overload: costs as map of runeId -> perCast
    ...
    // Returns shortfall for single cast (runeId -> missing amount)
    ...
    // Reads rune amounts from inventory for given IDs
    ...
    // Merge multiple rune-count maps
