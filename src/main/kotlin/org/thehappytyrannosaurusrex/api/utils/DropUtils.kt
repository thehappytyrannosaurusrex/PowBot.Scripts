...
object DropUtils {
    ...
    private const val SCRIPT_NAME = "DropUtils"
    ...
    fun enableTapToDrop(): Boolean {
        ...
        return tryEnableViaSettingsUI()
    }

    private fun tryEnableViaSettingsUI(): Boolean {
        if (!openSettingsTab()) {
    ...
    private fun searchForTapToDrop(): Boolean {
        ...
        try {
            org.powbot.api.Input.sendln("tap to")
        } catch (_: Throwable) {
            try {
                org.powbot.api.Input.send("tap to\n")
            } catch (e: Throwable) {
